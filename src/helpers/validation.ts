...
export const CheckoutValidation = (type) =>
  Yup.object().shape({
    creditCardNumber: Yup.string()
        .required("Digite um número de cartão válido")
        .matches(/^((4\d{3})|(5[1-5]\d{2})|(6011)|(34\d{1})|(37\d{1}))-?\s?\d{4}-?\s?\d{4}-?\s?\d{4}|3[4,7][\d\s-]{15}$/, "Digite um número de cartão válido"),
    ...
    creditCardCPF: Yup.string()
        ...
        .matches(/^([0-9]{3}\.?[0-9]{3}\.?[0-9]{3}\-?[0-9]{2}|[0-9]{2}\.?[0-9]{3}\.?[0-9]{3}\/?[0-9]{4}\-?[0-9]{2})$/, "Digite um número de cpf válido"),
